    ...
    public static class AnimLoader
    {
        public static void LoadAnimAssets(IUnrealEssentials unrealEssentials, Config configuration, string modLocation)
        {
            if (configuration.AnimTrue == AnimType.OriginalAnims)
                ...
                unrealEssentials.AddFromFolder(Path.Combine(modLocation, "3d", "Anims", "Custom Anims")); // aoa might be seperate from all of these maybe, depends on if we do custom ending poses per aoa 2d option. Would need to pass the 2d aoa conditionals if we do
